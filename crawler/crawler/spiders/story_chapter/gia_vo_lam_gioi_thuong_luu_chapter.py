...
class GiaVoLamGioiThuongLuuChapterSpider(scrapy.Spider):
    name = "gia_vo_lam_gioi_thuong_luu_chapter"
    allowed_domains = ["truyenqqgo.com"]
    start_urls = ["https://truyenqqgo.com/truyen-tranh/gia-vo-lam-gioi-thuong-luu-19853"]
    ...
    def parse_chapter(self, response):
        ...
        for idx, img_url in enumerate(image_urls):
            ...
            gcs_path = f"gia-vo-lam-gioi-thuong-luu/{chapter_name_en}/{idx+1}.jpg"
            storage_client = storage.Client.from_service_account_json(GCS_CREDENTIALS)
            bucket = storage_client.bucket(GCS_BUCKET)
            blob = bucket.blob(gcs_path)
            if blob.exists():
