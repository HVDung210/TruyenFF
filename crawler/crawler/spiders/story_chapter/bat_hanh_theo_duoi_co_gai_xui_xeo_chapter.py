...
class BatHanhTheoDuoiCoGaiXuiXeoChapterSpider(scrapy.Spider):
    name = "bat_hanh_theo_duoi_co_gai_xui_xeo_chapter"
    allowed_domains = ["truyenqqgo.com"]
    start_urls = ["https://truyenqqgo.com/truyen-tranh/bat-hanh-theo-duoi-co-gai-xui-xeo-19871"]
    ...
    def parse_chapter(self, response):
        ...
        for idx, img_url in enumerate(image_urls):
            ...
            gcs_path = f"bat-hanh-theo-duoi-co-gai-xui-xeo/{chapter_name_en}/{idx+1}.jpg"
            storage_client = storage.Client.from_service_account_json(GCS_CREDENTIALS)
            bucket = storage_client.bucket(GCS_BUCKET)
            blob = bucket.blob(gcs_path)
            if blob.exists():
