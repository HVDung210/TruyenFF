...
# Khởi tạo Storage client một lần
storage_client = storage.Client.from_service_account_json(GCS_CREDENTIALS)
bucket = storage_client.bucket(GCS_BUCKET)

def upload_to_gcs_from_bytes(data, gcs_path):
    """Upload ảnh từ bytes trực tiếp lên GCS."""
...
class DaiQuanGiaLaMaHoangSpider(scrapy.Spider):
    name = "dai_quan_gia_la_ma_hoang_chapter"
    allowed_domains = ["truyenqqgo.com"]
    start_urls = [
        "https://truyenqqgo.com/truyen-tranh/dai-quan-gia-la-ma-hoang-7015"
    ]
    ...
    def parse(self, response):
        ...
        # Đảo thứ tự để lấy chap cũ -> mới
        chapters = chapters[::-1]

        # Chỉ crawl 4 chap đầu tiên 
    ...
    def parse_chapter(self, response):
        ...
        for idx, img_url in enumerate(image_urls):
            ...
            gcs_path = f"dai-quan-gia-la-ma-hoang/{chapter_name_en}/{idx+1}.jpg"

            # Kiểm tra nếu ảnh đã tồn tại trên GCS
            blob = bucket.blob(gcs_path)
            if blob.exists():
                gcs_urls.append(blob.public_url)
                continue

            # Tải ảnh về và upload lên GCS
