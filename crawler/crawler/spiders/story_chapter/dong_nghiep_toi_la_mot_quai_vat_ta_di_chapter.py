...
class DongNghiepToiLaMotQuaiVatTaDinChapterSpider(scrapy.Spider):
    name = "dong_nghiep_toi_la_mot_quai_vat_ta_di_chapter"
    allowed_domains = ["truyenqqgo.com"]
    start_urls = ["https://truyenqqgo.com/truyen-tranh/dong-nghiep-toi-la-mot-quai-vat-ta-di-18774"]
    ...
    def parse_chapter(self, response):
        ...
        for idx, img_url in enumerate(image_urls):
            ...
            gcs_path = f"dong-nghiep-toi-la-mot-quai-vat-ta-di/{chapter_name_en}/{idx+1}.jpg"
            storage_client = storage.Client.from_service_account_json(GCS_CREDENTIALS)
            bucket = storage_client.bucket(GCS_BUCKET)
            blob = bucket.blob(gcs_path)
            if blob.exists():
