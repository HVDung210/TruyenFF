...
class TextDetectionService {
    ...
    /**
     * Detect text trong comic image bằng Cloud Vision API
    ...
    async detectTextInComic(imagePath) {
        try {
            console.log(`[TextDetectionService] Starting text detection for: ${imagePath}`);
            ...
            // Gọi Python script
            const command = `python "${this.scriptPath}" "${imagePath}" "${this.credentialsPath}"`;
            console.log(`[TextDetectionService] Executing command: ${command}`);
            ...
            if (stderr) {
                console.log(`[TextDetectionService] Python stderr: ${stderr}`);
            }

            // Parse JSON response
            const result = JSON.parse(stdout);
            
            console.log(`[TextDetectionService] Text detection completed successfully`);
            console.log(`[TextDetectionService] Found ${result.panelCount} panels, ${result.totalTextDetected} with text`);
    ...
    async detectTextInPanel(imagePath, panel) {
        try {
            console.log(`[TextDetectionService] Detecting text in panel:`, panel);
